...
static Blob make_blob(void) {
    Blob b; unsigned i,j; uint32_t colors[6]={0xff3060c8u,0xc0ff8030u,0xc0ffb030u,0xc0fff020u,0xc030c0ffu,0xc0a060ffu};
    memset(&b,0,sizeof(b)); memcpy(b.data,"FLCN64B\0",8);b.n=8;
    u32(&b,4);u32(&b,26);u32(&b,1);u32(&b,6);u32(&b,2);u32(&b,1);u32(&b,1);u32(&b,1);u32(&b,3);
    ...
    u16(&b,0);u16(&b,0);
    f32(&b,-1);f32(&b,0);f32(&b,0);f32(&b,0);f32(&b,0);
    f32(&b,1);f32(&b,0);f32(&b,0);f32(&b,1);f32(&b,0);
    f32(&b,0);f32(&b,4);f32(&b,0);f32(&b,.5f);f32(&b,1);
...
static int synthetic(void) {
    ...
    if(actual!=0xd57494b103e53883ULL){fprintf(stderr,"FAIL framebuffer hash drift\n");falcon_presentation_destroy(p);return 1;}
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr,"usage: %s [--blob falcon_runtime.bin output.bmp]\n",argv[0]);return 2;
}
