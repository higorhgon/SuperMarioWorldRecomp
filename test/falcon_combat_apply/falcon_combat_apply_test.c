...
static int s_sprite_calls, s_block_calls, s_expected_slot;
...
static ForeignAttackHitbox attack(void) {
    ...
    a.offset_x=50; a.width=300; a.height=300; a.flags=FOREIGN_ATTACK_BREAK_BLOCKS; return a;
}
...
int main(void) {
    ...
    /* Right-facing center is x=112: upright 16x24 catches low body. */
    install_sprite(3,0x0f,108,120); install_sprite(5,0x0f,110,110); s_expected_slot=3;
    memset(s_ram,0x5a,16); memcpy(scratch,s_ram,16); memset(&hit,0,sizeof(hit));
    smw_falcon_combat_apply(&cpu,&a,1,&hit);
    CHECK(s_sprite_calls==1 && s_ram[0x14cb]==2 && s_ram[0x14cd]==8 && hit.attack_connected);
    ...
    cpu=fresh(); put16(0x94,100); put16(0x96,100); install_sprite(2,0x04,108,128); s_expected_slot=2;
    calls=s_sprite_calls; smw_falcon_combat_apply(&cpu,&a,1,&(ForeignCollisionResult){0});
    CHECK(s_sprite_calls==calls+1 && s_ram[0x14ca]==2);
    cpu=fresh(); put16(0x94,100); put16(0x96,100); install_sprite(2,0x04,108,120); s_ram[0x14ca]=9;
    ...
    cpu=fresh(); put16(0x94,100); put16(0x96,100); install_sprite(1,0x0f,96,110); s_expected_slot=1;
    smw_falcon_combat_apply(&cpu,&a,-1,&(ForeignCollisionResult){0}); CHECK(s_ram[0x14c9]==2);

    cpu=fresh(); put16(0x94,100); put16(0x96,100); put16(0x9a,104); put16(0x98,112);
    ...
    cpu=fresh(); put16(0x94,100); put16(0x96,100); put16(0x9a,104); put16(0x98,112); s_ram[0x1693]=0x21;
