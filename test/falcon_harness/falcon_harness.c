...
#include "../../src/mods/falcon/captain_falcon_controller.h"
...
/* Keep the game-owned boundary honest as well as the source-unit machine.
 * The scripted traces below intentionally work at the lower layer so their
 * assertions can inspect FalconFighter directly; this smoke covers the
 * normalized-input, hitbox, audio, collision, and save delegates. */
static int run_controller_smoke(void)
{
    SMWCaptainFalconController controller;
    SMWForeignInput input;
    SMWForeignMove move;
    SMWForeignCollision collision;
    SMWForeignState state;
    uint8_t blob[1 + sizeof(FalconFighter)];
    int len;

    memset(&input, 0, sizeof(input));
    smw_captain_falcon_reset(&controller, &state);
    input.special_pressed = 1;
    smw_captain_falcon_tick(&controller, &state, &input, &move);
    if (move.state != FL_FALCON_PUNCH_GROUND ||
        move.audio_cues != FALCON_AUDIO_CUE_BIT(FALCON_AUDIO_PUNCH_FALCON)) {
        fprintf(stderr, "FAIL controller adapter did not publish Falcon Punch\n");
        return 1;
    }

    memset(&collision, 0, sizeof(collision));
    collision.actual_dx = move.requested_dx;
    collision.actual_dy = move.requested_dy;
    collision.grounded = 1;
    smw_captain_falcon_resolve(&controller, &state, &collision);

    len = smw_captain_falcon_serialize(&controller, blob, (int)sizeof(blob));
    if (len < 0 || !smw_captain_falcon_deserialize(&controller, blob, len)) {
        fprintf(stderr, "FAIL controller adapter save roundtrip\n");
        return 1;
    }
    puts("controller adapter smoke: PASS");
    return 0;
}
...
int main(int argc, char **argv)
{
    ...
    if (run_controller_smoke()) return 1;
