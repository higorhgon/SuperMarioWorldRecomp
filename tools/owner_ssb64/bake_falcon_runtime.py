...
except ModuleNotFoundError:  # Keep provenance/cache verification usable without Pillow.
    Image = None
...
def write_blob(root: Path, output: Path):
    if Image is None:
        raise RuntimeError(
            "bake_falcon_runtime requires Pillow to decode owner-derived textures; "
