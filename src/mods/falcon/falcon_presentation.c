...
int falcon_presentation_draw(const FalconPresentation*p,const FalconPresentationPose*pose,const FalconPresentationTarget*t){
    ...
    for(i=0;i<p->triangles_n;i++){
        ...
        for(j=0;j<3;j++){float z,x,y,pt[3],a,ca,sa,px,py;point(w[q->joint],q->v[j].p,pt);a=t->yaw_degrees*3.14159265358979323846f/180.f;ca=cosf(a);sa=sinf(a);px=pt[0]*ca+pt[2]*sa;z=-pt[0]*sa+pt[2]*ca;x=(px-(lo[0]+hi[0])*.5f)*dir;y=pt[1]-lo[1];py=(-y-z*.08f-foot)*scale;d->v[j].x=t->anchor_x+x*scale+z*scale*.18f;d->v[j].y=t->anchor_y+py;d->v[j].z=z;d->v[j].u=q->v[j].uv[0];d->v[j].v=q->v[j].uv[1];d->depth+=z;}
        d->depth/=3.0f;
    }
